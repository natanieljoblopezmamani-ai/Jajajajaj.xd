...
local ZONAS_EFECTO = {
    cabeza  = {"frente","sien","ojo","mandíbula","mejilla","oreja","nuca","tabique","labio","barbilla","pómulo","arco","casco"},
    cuello  = {"cuello","tráquea","nuez"},
    torso   = {"pecho","esternón","clavícula","escápula","costilla","placa pectoral","placa dorsal","kevlar","chaleco","axila","hombro"},
    abdomen = {"abdomen","ombligo","plexo","ingle","zona lumbar","sacro"},
    brazo   = {"brazo","bíceps","tríceps","codo","antebrazo","muñeca","mano","dedos","codiera","guante"},
    pierna  = {"muslo","cuádriceps","isquiotibial","rodilla","pantorrilla","tobillo","rodillera"},
    pie     = {"pie"},
    protegido = {"chaleco","kevlar","casco","placa","blindaje","codiera","rodillera","guante táctico"},
}
...
local function getZona(parte)
    local p = parte:lower()
    for zona, palabras in pairs(ZONAS_EFECTO) do
        if zona ~= "protegido" then
            for _, palabra in ipairs(palabras) do
                if p:find(palabra, 1, true) then return zona end
            end
        end
    end
    return "general"
