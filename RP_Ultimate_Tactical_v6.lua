-- ============================================================
--  RP ULTIMATE FULL  |  TACTICAL EDITION  v6.0
--  Golpes: sin "de objetivo", solo acción pura. Si entra alguien → [entra Nombre]
--  Disparos: sin nombre si campo vacío, con nombre si lo escribes
--  Autos: conductor OPCIONAL, diagrama de auto
--  AR-15 breve y profesional
-- ============================================================
...
-- PROCESAR ACCIÓN
-- ============================================================
local function Procesar(parte, tipo, extraVic)
    local vic      = victimaInput.Text ~= "" and victimaInput.Text or ""
    ...
    if tipo=="apuntar" then
        -- Con nombre: "-apunta a [parte] de [vic]-"
        -- Sin nombre: "-apunta a [parte]-"
        if vic ~= "" then
            EnviarAlChat("-apunta a " .. parte .. " de " .. vic .. "-")
        else
            EnviarAlChat("-apunta a " .. parte .. "-")
        end

    elseif tipo=="disparar" then
        -- Con nombre: "-Dispara 1 bala (x) al [parte] de [vic]-"
        -- Sin nombre: "-Dispara 1 bala (x) al [parte]-"
        local verbo = verbos[math.random(#verbos)]
        if vic ~= "" then
            EnviarAlChat("-" .. verbo .. " 1 bala (" .. bal .. ") al " .. parte .. " de " .. vic .. "-")
        else
            EnviarAlChat("-" .. verbo .. " 1 bala (" .. bal .. ") al " .. parte .. "-")
        end

    elseif tipo=="vehiculo_apuntar" then
        -- extraVic = conductor OPCIONAL
    ...
    elseif tipo=="vehiculo_disparar" then
        local cond  = (extraVic and extraVic ~= "") and (" del vehículo de " .. extraVic) or ""
        local verbo = verbos[math.random(#verbos)]
        EnviarAlChat("-" .. verbo .. " 1 bala (" .. bal .. ") en la " .. parte .. cond .. "-")

    elseif tipo=="combate" then
        -- Sin ningún campo: solo "-[acción]-"
        -- extraVic = quién se mete (OPCIONAL) → " [entra Nombre]" al final
        local sufijo = (extraVic and extraVic ~= "") and (" [entra " .. extraVic .. "]") or ""
        EnviarAlChat("-" .. parte .. sufijo .. "-")

    else
        -- Médico / señal / genérico — sin nombre
        EnviarAlChat("-" .. parte .. "-")
    end
end
