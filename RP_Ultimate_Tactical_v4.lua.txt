-- ============================================================
--  RP ULTIMATE FULL  |  TACTICAL EDITION  v4.0
--  Correcciones: sidebar táctil, minimizar, diagrama corporal,
--  candado en botón flotante, más acciones detalladas
-- ============================================================
...
-- ============================================================
-- DIAGRAMA CORPORAL  (ImageLabel + punto dinámico)
-- Se muestra al hacer hover sobre un bloque
-- ============================================================
-- Silueta corporal dibujada con Lines/Frames (liviana, sin imagen externa)
local DiagramPanel = Instance.new("Frame", Main)
DiagramPanel.Size=UDim2.new(0,90,0,180); DiagramPanel.Position=UDim2.new(1,-94,0,46)
...
local DiagLabel = Instance.new("TextLabel", DiagramPanel)
DiagLabel.Size=UDim2.new(1,0,0,14); DiagLabel.BackgroundTransparency=1
DiagLabel.Text="ZONA"; DiagLabel.TextColor3=C.textDim; DiagLabel.TextSize=8
DiagLabel.Font=Enum.Font.GothamBold; DiagLabel.TextXAlignment=Enum.TextXAlignment.Center

-- Silueta con frames (cabeza + torso + brazos + piernas)
local function mkSilFrame(parent,x,y,w,h,col)
    local f=Instance.new("Frame",parent)
    f.Size=UDim2.new(0,w,0,h); f.Position=UDim2.new(0,x,0,y)
    f.BackgroundColor3=col or C.surfaceAlt; f.BorderSizePixel=0
    corner(f,3); return f
end
-- Head
mkSilFrame(DiagramPanel,30,14,30,28,Color3.fromRGB(40,48,58))
-- Neck
mkSilFrame(DiagramPanel,38,42,14,8,Color3.fromRGB(35,42,52))
-- Torso
mkSilFrame(DiagramPanel,22,50,46,52,Color3.fromRGB(40,48,58))
-- Left arm
mkSilFrame(DiagramPanel,10,50,12,46,Color3.fromRGB(36,44,54))
-- Right arm
mkSilFrame(DiagramPanel,68,50,12,46,Color3.fromRGB(36,44,54))
-- Left leg
mkSilFrame(DiagramPanel,22,102,20,68,Color3.fromRGB(40,48,58))
-- Right leg
mkSilFrame(DiagramPanel,48,102,20,68,Color3.fromRGB(40,48,58))

-- Punto dinámico (rojo)
local DiagDot = Instance.new("Frame", DiagramPanel)
DiagDot.Size=UDim2.new(0,9,0,9); DiagDot.BackgroundColor3=C.dot
DiagDot.BorderSizePixel=0; DiagDot.ZIndex=10
corner(DiagDot,5)
-- Anillo exterior del punto
local DotRing = Instance.new("Frame", DiagramPanel)
DotRing.Size=UDim2.new(0,15,0,15); DotRing.BackgroundTransparency=1
DotRing.BorderSizePixel=0; DotRing.ZIndex=9
local ringStroke=Instance.new("UIStroke",DotRing)
ringStroke.Color=C.dot; ringStroke.Thickness=1.5; ringStroke.Transparency=0.3
corner(DotRing,8)

local DiagZoneTxt = Instance.new("TextLabel", DiagramPanel)
DiagZoneTxt.Size=UDim2.new(1,-4,0,24); DiagZoneTxt.Position=UDim2.new(0,2,1,-28)
...
local function showDiagram(zona)
    local coord = bodyMap[zona:lower()] or DEFAULT_DOT
    local px = coord[1]
    local py = coord[2]
    -- Convertir a posición en píxeles dentro del diagrama (90×180)
    local W,H = 90,180
    DiagDot.Position  = UDim2.new(0, math.floor(px*W)-4, 0, math.floor(py*H)-4)
    DotRing.Position  = UDim2.new(0, math.floor(px*W)-7, 0, math.floor(py*H)-7)
...
-- ============================================================
local layoutOrder = 0
local function crearBloque(texto, tipo, catAccentColor)
    layoutOrder = layoutOrder + 1
    local esCombate = (tipo=="combate")
    local isMedical = (tipo=="medical")
    local isSignal  = (tipo=="signal")
    local isScript  = (tipo=="script")

    local hdrCol = catAccentColor or
        (esCombate and C.combat) or
        (isMedical and C.medical) or
        (isSignal  and C.signal)  or
        (isScript  and C.script_c) or
        C.accent

    local f = Instance.new("Frame", Scroll)
    f.Size=UDim2.new(1,-2,0,82); f.BackgroundColor3=C.surfaceAlt
    f.BorderSizePixel=0; f.Name=texto; f.LayoutOrder=layoutOrder
    corner(f,7); stroke(f,C.border,1,0.55)

    -- Hover → mostrar diagrama
    f.MouseEnter:Connect(function() showDiagram(texto) end)
    ...
    -- Botones según tipo
    if not esCombate and not isMedical and not isSignal and not isScript then
        ...
        b.TextSize=10; b.Font=Enum.Font.GothamSemibold; b.BorderSizePixel=0
        corner(b,6)
        if esCombate then
            b.Text="👊  EJECUTAR"; b.BackgroundColor3=C.combatDim; b.TextColor3=Color3.fromRGB(190,140,255)
            hoverBtn(b,C.combatDim,C.combat)
        elseif isMedical then
            b.Text="💊  APLICAR"; b.BackgroundColor3=C.medicalDim; b.TextColor3=Color3.fromRGB(255,140,160)
            hoverBtn(b,C.medicalDim,C.medHover)
        elseif isSignal then
            b.Text="🤟  SEÑALAR"; b.BackgroundColor3=C.signalDim; b.TextColor3=Color3.fromRGB(255,220,100)
            hoverBtn(b,C.signalDim,C.signalHov)
        elseif isScript then
            b.Text="📋  ENVIAR"; b.BackgroundColor3=C.scriptDim; b.TextColor3=Color3.fromRGB(100,190,255)
            hoverBtn(b,C.scriptDim,C.scriptHov)
        end
...
local armasData = {
    ["AR-15"] = {
        icon="🔫",
        pasos={
            "-saca el upper receiver y lower receiver del maletín con cuidado-",
            "-coloca el lower receiver sobre una superficie plana y estable-",
            "-une el upper receiver con el lower receiver alineando las guías-",
            "-inserta el pasador delantero (pivot pin) empujando de izquierda a derecha-",
            "-inserta el pasador trasero (takedown pin) hasta que encaje con clic-",
            "-verifica que ambos pasadores estén completamente insertados-",
            "-carga 30 balas 5.56×45mm NATO en el cargador de 30 rondas-",
            "-golpea la base del cargador con la palma para asentar las balas-",
            "-inserta el cargador firmemente en el pozo del lower hasta escuchar el clic-",
            "-jala la manija de carga hacia atrás con la mano no dominante-",
            "-suelta la manija de carga para chambear 1 bala en la recámara-",
            "-verifica visualmente por la ventana de expulsión que hay bala en recámara-",
            "-activa el selector de fuego en posición SAFE-",
            "-enciende la mira holográfica (EOTech/Aimpoint)-",
            "-activa el láser PEQ-15 (modo visible o IR según el entorno)-",
            "-rosca el silenciador (suppressor) en el cañón si la misión lo requiere-",
            "-extiende y ajusta la culata telescópica a la longitud del brazo-",
            "-realiza verificación final: cargador insertado, bala en recámara, seguro activo-",
            "-declara: arma lista y en condición 1-",
            "-desactiva el seguro (FIRE) únicamente antes de disparar-",
        }
    }
}
...
for cat,lista in pairs(database) do
    for _,v in ipairs(lista) do
        crearBloque(v, tipoMap[cat] or "normal")
    end
end
